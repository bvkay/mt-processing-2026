...
# of `mtproc.masks.normalise` as `mtproc.masks.utc` does, so they are passed
...
def _drop_masked_windows(band, dec_level_config, X, Y, RR, masks, min_windows: int, log: _BandMaskLog):
    ...
    A mask covers the band when `mtproc.masks.applies` is True at the band's
...
def _band_masks_applied(masks, min_windows: int = MIN_MASKED_WINDOWS):
    ...
    Args:
        masks (list of dict): A site's masks (`mtproc.masks.load_masks`).
...
def build_config(kd, band_scheme: dict | None = None, tweaks: dict | None = None, **config_kwargs):
    ...
    Args:
        ...
        band_scheme (dict, optional): Band scheme from `mtproc.bands`.
    ...
    **config_kwargs,
    ...
    Args:
        ...
        out_dir (Path, optional): Folder for the EDI; nothing is written
            when None.
        min_run_seconds (float): Runs shorter than this are dropped.
        band_scheme (dict, optional): Dict from `mtproc.bands` (band_edges,
        ...
        time_masks (list of dict, optional): The masks of the local and
            remote sites (`mtproc.masks.load_masks`, `mtproc.masks.union_masks`).
