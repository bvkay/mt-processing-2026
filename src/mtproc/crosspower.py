...
`mtproc.bands.build_band_scheme` returns. The GUI's Cross-powers tab draws
the estimates against time and in the (log10 |Z|, phase) plane, where a
stretch that sits apart from the others can be masked (`mtproc.masks`).
...
levels (`mtproc.timefreq.decimation_levels`); each channel's spectra are
...
def _roles(names, wanted) -> dict[str, str]:
    ...
    one of `mtproc_gui.channels.roles`.
...
def band_table(band_scheme: dict):
    ...
    Args:
        band_scheme (dict): The survey's band scheme (see mtproc.bands).
...
def compute_windows(local_h5, station: str, remote_h5, remote_station: str, start=None, end=None,
    ...
    if band_scheme is None:
        raise ValueError("band_scheme is required: mtproc.bands.build_band_scheme(sample_rate, **processing)")
...
def _estimate(sums: dict, n_used: np.ndarray, n_possible: np.ndarray, n_harm: np.ndarray) -> dict:
    ...
    `mtproc.timefreq.window_spectra` applies.
...
def masked_chunks(result: dict, masks, j: int) -> np.ndarray:
    ...
    levels) is masked when a mask covering band j (`mtproc.masks.applies`,
...
def stack_impedance(result_or_store, masks=(), chunk_s: float | None = None) -> dict:
    ...
    Args:
        ...
        masks: Mask dicts (`mtproc.masks`); normalised here.
