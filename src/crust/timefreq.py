...
`crust.bands.build_band_scheme` lays out for processing: the FFT length is
...
- `load_station`    every run of a station concatenated onto one sample grid,
                    ...
                    local station's grid, which is what `crust.qc.align` does
...
# leaves a window with fewer NaN, and `crust.crosspower` uses it as its compute floor
MIN_WINDOWS = 4
...
def _real_runs(station_group) -> list[tuple[str, pd.Timestamp, pd.Timestamp]]:
    ...
    1980-01-01; they are skipped, as `crust.qc.longest_run` skips them.
...
def window_spectra(
    ...
    Returns:
        ...
        dropped for its gaps), which `crust.crosspower` reports as a
...
def decimation_levels(
    ...
    and by `crust.crosspower` for a chunk's impedances. While a level is
