...
(`crust.instruments.read_run`) with the reader's channel names and filter
...
def _donor_channel(survey: Survey, donor_site: str, comp: str, t0, n: int, fs: float, tag: str) -> np.ndarray:
    ...
    Raises:
        ...
        ValueError: If no donor run covers the span, the sample rate
            ...
            (the GPS timing assumption, as in `crust.virtual`), or the run
            is too short.
    """
...
def build_variant(survey: Survey, site_name: str, workers: int = 4) -> Path:
    ...
    each run's channel arrays (`crust.noise.apply_filters_arrays`, with
...
def select_files(site_dir: Path, start=None, end=None, instrument: str = "lemi423") -> list[Path]:
    ...
    `crust.instruments.file_start` for the others). A file's span runs to
...
def ingest_site(
    ...
    mt-io readers (`crust.instruments.read_run`), keeping the reader's
