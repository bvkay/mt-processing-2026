# -*- coding: utf-8 -*-
"""
Recorders CRUST reads and their detection from a folder of raw files
...
`scripts/new_survey.py` as the site's own `instrument:`.
    ...
    lemi423  LEMI-423 broadband logger: `<unix epoch>.B423` binary files
             ...
             LEMI-120 coil response and `h_scale` are added in crust.ingest)
...
importing this module (and `crust.survey`, which imports it) does not load
...
def read_run(instrument: str, files: list[Path], site, site_dir: Path, calibration: Path | None = None):
    ...
    LEMI-423 files are read in `crust.ingest.ingest_site`.
...
def add_electric_gain(run, gain: float, tag: str = "") -> list[str]:
    ...
    list, as `crust.ingest._apply_h_scale` adds its scale, so the archive
...
def recorder_ini_high_gain(site_dir: Path) -> list[str] | None:
    ...
    and `crust.ingest._electric_gain` uses the result for a warning.
