...
raw files of an instrument CRUST reads (`INSTRUMENTS`: LEMI-423, LEMI-424,
...
from .instruments import INSTRUMENTS, detect_instrument  # noqa: F401  (crust.survey.INSTRUMENTS)
...
# `channels:`, applied at ingest (crust.ingest._keep_channels). An
...
class SiteConfig:
    ...
    Attributes:
        ...
        sensor_type (str or None): EDL (Earth Data PR6-24) sites: the
            ...
            (crust.instruments.EDL_SENSORS). "bartington" is Mag-03
        ...
        electric_gain (float): EDL sites: extra gain of the electric chain
            ...
            (crust.instruments.read_run). A `defaults:` value applies to the
        ...
        flip_reversed_dipoles (bool): How to read a 180 or 270 deg dipole
            ...
            (crust.compare.phase_quadrants); a wrong choice puts one mode
            180 deg out.
        ...
        filters (list of dict or None): Declared time-domain filters applied
            at ingest, in order (see crust.noise), from
            <survey>/filters.yaml.
...
class Survey:
    ...
    def processing(self) -> dict:
        """Band/period targets for aurora (kwargs for crust.bands schemes)."""
    ...
    def sample_rate_of(self, site: str) -> float:
        ...
        scripts/fetch_observatory.py) is at `crust.observatory.FS`; every
