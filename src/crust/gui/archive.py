...
windows (`crust.gui.windows`), and `crust.gui.segment.load_segment` uses it
...
last run's end. This is the same concatenation `crust.timefreq.load_station`
...
chain, `crust.timefreq._scalar_gain`. This leaves the electrics fully
...
Channels keep the archive's own names (`crust.gui.channels`). `load_grid`
...
`crust.gui.windows.window_list` walks it to find a station's windows and
...
from crust.gui import channels
