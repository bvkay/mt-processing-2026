...
``python -m crust``, ``python -m crust.gui`` and the ``crust`` console script
all run `main`. The optional argument is a survey config, for example
...
`crust.gui.theme` (dark grey surface, channel colours) is applied to the
...
from crust.gui.app import MainWindow
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m crust",
        description="Open the CRUST GUI, on a survey when one is given.",
    )
