...
* `compute_preview` has no Qt dependency and copies only what it must. The
  window's arrays go through `crust.noise.apply_filters_arrays`, the same
  function ingest calls, on four threads. `crust.timefreq.psd_ladder` then
...
* `FilterPreview` runs one `PreviewWorker` at a time. The latest request
  ...
  (`crust.gui.segment.load_segment`) under `State.archive_lock`; a donor
  without an archive is reported as a provenance line.

`crust.gui.filter_views` draws a `PreviewResult`. Results are held in memory
...
from crust.timefreq import psd_ladder
from crust.gui import theme
from crust.gui.segment import REMOTE_COMPS, load_segment
