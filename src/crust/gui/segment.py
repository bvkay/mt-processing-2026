...
`crust.timefreq` (`levels_plan`, `cascade`, `band_from_levels`,
...
* `Segment`: one stretch in `crust.timefreq.Record`'s convention, float32
  ...
  `to_record()` returns a `Record`, so `crust.timefreq.merge` folds a remote
  in unchanged.
* `load_segment`: reads a segment read-only through the run grid of
  `crust.gui.archive` (`load_grid`, `run_slices`), one channel at a time.
...
`crust.timefreq.LOCAL_PAIRS` and `REMOTE_PAIRS`, with each LEMI-423 name
resolved to the channel playing that role (`crust.gui.channels.roles`: the
...
The module has no Qt dependency. `crust.gui.segment_store` runs it off the
...
from crust.gui.archive import load_grid, run_slices
from crust.gui.channels import REMOTE_COMPS, order, resolve_pairs, roles  # noqa: F401  (REMOTE_COMPS: the store's)
...
@dataclass
class Segment:
    """One stretch of a station, in `crust.timefreq.Record`'s convention.
...
def load_segment(
    ...
    The segment sits on the station's own grid (`crust.gui.archive.load_grid`):
...
def compute_segment_qc(
    ...
    Every number comes from `crust.timefreq`; this function chooses the
