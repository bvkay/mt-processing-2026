...
2. The list of the site's filters in the order they are applied, with Add
   ...
   with a non-empty list; `crust.gui.copy_filters`, for surveys where many
...
3. The preview (`filter_preview`): whenever the list or a form value changes
   ...
   `crust.noise.apply_filters_arrays` off the GUI thread. It is drawn raw
...
(`crust.ingest.processing_archive`, `build_variant`); the raw recording is
...
from crust.survey import INSTRUMENTS
from crust.gui.copy_filters import CopyFiltersDialog
from crust.gui.filter_forms import CHANNELS, LABELS, make_forms, summarise
from crust.gui.filter_preview import FilterPreview
from crust.gui.filter_views import PreviewPane
from crust.gui.window_chooser import WindowChooser
...
class FiltersTab(QWidget):
    ...
    def _variant_paths(self, site: str | None) -> list:
        ...
        Normally there is at most one, since `crust.ingest.build_variant`
        prunes the others.
        """
