...
computes a window's STFT windows once (`mtproc.crosspower.compute_windows`)
and regroups them (`mtproc.crosspower.bin_windows`) for every chunk length
and every window inside the computed one; the masks are saved per site in
`<survey>/masks.yaml` (`mtproc.masks.save_masks`) and act in processing and
in `mtproc.crosspower.stack_impedance`.
...
from mtproc.crosspower import (
    BIN_S, CHUNKS_S, band_table, band_view, bin_windows, compute_windows, level_multiples, masked_chunks,
)
from mtproc.masks import applies, iso, load_masks, normalise, save_masks, utc
...
def span_text(start, end, seconds: bool = False) -> str:
    ...
    Args:
        start: The span's start (anything `mtproc.masks.utc` takes).
        end: The span's end.
...
def processing_source(survey, site: str, raw: Path) -> tuple[Path, str]:
    ...
    missing (`mtproc.ingest.processing_archive` is the call that builds one).
...
class CrossPowerTab(QWidget):
    ...
    def applies(self, mask: dict, j: int) -> bool:
        """Return whether `mask` covers band j (`mtproc.masks.applies` at its centre period)."""
    ...
    def masked_chunks(self, j: int) -> np.ndarray:
        ...
        `mtproc.crosspower.masked_chunks`, judged on the windows or minute
        bins `stack_impedance` keeps.
        """
        return masked_chunks(self.result, self.masks, j)

    def view(self, j: int) -> dict:
        """Return band j of the result on its own grid (`mtproc.crosspower.band_view`)."""
