...
the `bbmt.survey.Survey` built from it, the selected site) plus the two things
...
`State` also owns the window's **one** `JobRunner`. Every tab that launches a
script queues on `state.runner`, so the whole GUI runs exactly one job at a
time: an MTH5 must never be open in two processes (HDF5 locking). The queue
table and the script log live on the Process tab, in one `queue_table.QueuePanel`
showing every job from every tab; the other tabs show a one-line status only.
...
(`segment_store`, `bbmt_gui.segment_store`), the `ArchiveLock` that keeps
...
`MainWindow` is a QTabWidget over the eight tabs in the MATLAB app's order --
QC first (Metadata, Time Series, Spectra, Spectrogram, Coherence), then
Filter Data, then Process, then View EDIs -- plus a status bar and a File
menu, and, under the tabs in a vertical `QSplitter`, the `console.ConsoleStrip`
the owner asked for (fed the runner's log, loguru and `qc_started`; detail in
`console.py`).
"""
...
from bbmt_gui.console import ConsoleStrip, LoguruQtSink
from bbmt_gui.jobs import JobRunner
from bbmt_gui.reader import ArchiveLock
from bbmt_gui.segment_store import SegmentStore

# src/bbmt_gui/app.py -> repo root
...
class State(QObject):
    ...
    def set_selection(self, station: str, start, end) -> None:
        ...
        if self.selection is None or station != self.selection[0]:
            # the segment QC runs on the local pairs by default (Ben, 2026-09-22):
            # the remote pairs answer one narrower question (dead coil or quiet
            # field?) and are switched on from the Coherence tab when needed.
            # The Process tab still presets the station's declared remote.
            self.remote = None
    ...
    def archived_sites(self) -> list[str]:
        ...
        if d is None or not d.exists():
            return []
        return sorted(p.stem for p in d.glob("*.h5"))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, survey_yaml: str | Path | None = None, parent=None):
        ...
        from bbmt_gui.tabs.coherence import CoherenceTab
        from bbmt_gui.tabs.edis import EdiTab
        from bbmt_gui.tabs.filters import FiltersTab
        from bbmt_gui.tabs.metadata import MetadataTab
        from bbmt_gui.tabs.process import ProcessTab
        from bbmt_gui.tabs.spectra import SpectraTab
        from bbmt_gui.tabs.spectrogram import SpectrogramTab
        from bbmt_gui.tabs.timeseries import TimeSeriesTab

        self.setWindowTitle("BBMT processing")
        self.state = State()
    ...
    def show_edi(self, path: Path) -> None:
        ...
        station = path.stem.split("_rr-")[0]  # <station>_rr-<remote>[_w...].edi
