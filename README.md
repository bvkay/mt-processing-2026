...
python tests/campaign_unit.py        # campaign.py: plan, overlap rule, stacks, resume, dry-run counts, runner, filter check, masks signature, a MANTLE stage 3 config (its memory gate and dry-run hours, the ledger's engine, its shorter product scored over its own periods, through a stand-in process_rr.py), the per-site remotes, modes and windows (the override, xy-only scores, the window run and the merged product)
