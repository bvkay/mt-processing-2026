...
python tests/campaign_unit.py        # campaign.py: plan, overlap rule, stacks, resume, dry-run counts, runner, filter check, masks signature, a MANTLE stage 3 config (its memory gate and dry-run hours, the ledger's engine, its shorter product scored over its own periods, through a stand-in process_rr.py)
...
python tests/dc_level_unit.py        # dc_level_check.py on a synthetic archive: a channel at 1.6e9 counts flagged as an open input, healthy channels not, the rail rule, derived and observatory sites skipped, exit 2 for a missing archive
